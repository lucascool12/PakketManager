...
public class DataBean implements DataBeanRemote {
    ...
    public ArrayList<Object> getPakketen(int knr) {
        Query q = em.createQuery("select p from Pakketen p where p.knr = ?");
    ...
    @Override
    public void setPakketSatus(Object pakket, int status) {
    ...
    public void setPakketStatus(int pnr, int status) {
        this.setPakketSatus(this.getPakket(pnr), status);
    }
    ...
    @Override
    public void addPakket(int pgewicht, int pstatus, String lnaam, String lstraat, int lnummer, int lpostcode, String lgemeente) {
